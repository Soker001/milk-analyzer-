...
def main():
    ...
    if not os.path.exists(template_path):
        print(f"Template not found at {template_path}. Generating template in memory...")
        wb = openpyxl.Workbook()
        # Set up E2E Test Results sheet
        ws_results = wb.active
        ws_results.title = 'E2E Test Results'
        ws_results.append(['Test ID', 'Category', 'Test Name', 'Description', 'Status', 'Details', 'Timestamp'])
        # Set up Summary sheet
        ws_summary = wb.create_sheet('Summary')
        # Setup basic headers
        ws_summary['A14'] = 'Category Breakdown'
        ws_summary['B14'] = 'Pass'
        ws_summary['C14'] = 'Fail'
    else:
